...
void osc_clk_init(void)
{
    ...
    /* RFI_CON |= (1<<1);               // BT_EN */
    delay(10);
    SFR(WLA_CON13, 5,  5,  0); // BTOSC_CLSEL
    SFR(WLA_CON13, 10, 5,  0); // BTOSC_CRSEL

    if(BTOSC_CLK> 12)
    {
      SFR(WLA_CON14, 1, 4,  4); // BTOSC_Hcs
    }
    else
    {
      SFR(WLA_CON14, 1, 4,  2); // BTOSC_Hcs
    }

    SFR(WLA_CON15, 11, 1,  0); // BTOSC_EN_SEL_BASEBAND
    SFR(WLA_CON13, 15, 1,  1); // BTOSC_EN
    SFR(WLA_CON14, 5, 1,  1); // BTOSC_BT_OE
    SFR(WLA_CON14, 6, 1,  1); // BTOSC_FM_OE
    SFR(WLA_CON14, 7, 1,  1); // BTOSC_SYS_OE
