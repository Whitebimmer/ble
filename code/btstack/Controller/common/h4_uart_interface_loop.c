...
#define UART_RX_INDEX_IS_EXCEED(x)      (x >= UART_RX_MAX_SIZE)
#define UART_RX_BUF_REMAIN(x)           (UART_RX_MAX_SIZE - x)
...
{
    if (UART_RX_INDEX_IS_EXCEED(uart_rx_t.wr_index))
    {
        /* printf("UART_RX_BUF_IS_EXCEED : %x\n", uart_rx_t.wr_index); */
        /*-TODO-*/
        /* return; */
        //option : loop back
        uart_rx_t.wr_index = 0; 
    }
    uart_rx_t.buf[uart_rx_t.wr_index++] = data; 
    uart_rx_t.cnt++;
}

static void __data_check(char *buf, char offset, int len)
{
    u8 rd_index = ((uart_rx_t.rd_index + offset) % UART_RX_MAX_SIZE);

    if (len > UART_RX_BUF_REMAIN(rd_index))
    {
        /* puts("UART_RX_BUF not enough\n"); */
        /* return 0; */
        /* printf("remain %x, len %x\n", UART_RX_BUF_REMAIN(rd_index), len); */
        memcpy(buf, uart_rx_t.buf + rd_index, UART_RX_BUF_REMAIN(rd_index));
        buf += UART_RX_BUF_REMAIN(rd_index);
        len -= UART_RX_BUF_REMAIN(rd_index);
        rd_index = 0;
    }

    /* printf("len %x\n", len); */
    memcpy(buf, uart_rx_t.buf + rd_index, len);
...
{
    if (len > UART_RX_BUF_REMAIN(uart_rx_t.rd_index))
    {
        /* puts("UART_RX_BUF not enough\n"); */
        /* return 0; */
        printf("remain %x, len %x\n", UART_RX_BUF_REMAIN(uart_rx_t.rd_index), len);
        memcpy(buf, uart_rx_t.buf + uart_rx_t.rd_index, UART_RX_BUF_REMAIN(uart_rx_t.rd_index));
        buf += UART_RX_BUF_REMAIN(uart_rx_t.rd_index);
        len -= UART_RX_BUF_REMAIN(uart_rx_t.rd_index);
        uart_rx_t.cnt -= UART_RX_BUF_REMAIN(uart_rx_t.rd_index);
        uart_rx_t.rd_index = 0;
    }
...
void h4_uart_data_rxloop(void)
{
    ...
    __data_check(&packet_type , H4_ACL_FORMAT_PACKET_TYPE, 1);
    ...
    case HCI_COMMAND_DATA_PACKET:
        ...
        __data_check(packet_length, H4_COMMAND_FORMAT_LENGTH, 1);
        /* printf("except %x / %x\n", packet_length[0] + H4_COMMAND_FORMAT_PACKET_PAYLOAD, uart_rx_t.wr_index); */
        //command complete launch process
        if (packet_length[0] + H4_COMMAND_FORMAT_PACKET_PAYLOAD <= uart_rx_t.cnt)
        {
    ...
    case HCI_ACL_DATA_PACKET:
        ...
        __data_check(packet_length, H4_ACL_FORMAT_LENGTH , 2);
        /* printf("except %x / %x\n", READ_BT_16(packet_length, 0) + H4_ACL_FORMAT_PACKET_PAYLOAD, uart_rx_t.wr_index); */
        //ACL complete launch process
        if (READ_BT_16(packet_length, 0) + H4_ACL_FORMAT_PACKET_PAYLOAD <= uart_rx_t.cnt)
        {
...
void h4_uart_dump(void)
{
    printf("h4 uart dump : rd %x / wr %x / cnt %x\n", uart_rx_t.rd_index, uart_rx_t.wr_index, uart_rx_t.cnt);
